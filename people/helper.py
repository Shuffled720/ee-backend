...
def convertToAlumni(year):
    ...
    if (len(btech)!=0):
        for i in btech:
            Alumni.objects.create(name=i.name, roll_no=i.roll_no, program='B.Tech.', year=year, image=i.image)
            BTech.objects.get(roll_no=i.roll_no).delete()
    if (len(mtech)!=0):
        for i in mtech:
            Alumni.objects.create(name=i.name, roll_no=i.roll_no, program='M.Tech.', year=year, image=i.image)
